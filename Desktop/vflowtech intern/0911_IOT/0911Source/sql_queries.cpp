...
{		
	...
	sql = "INSERT INTO dataLog (uptime,timeStamp,primary_discharge_relay,primary_positive_pump,primary_negative_pump,balancing_valve,positive_valve,negative_valve,state_of_charge,bcu_voltage,bcu_current,bcu_power,bcu_state_of_charge,smoke_sensor,bcu_ocv,bcu_positive_tank_temp,bcu_negative_tank_temp,positive_tank_high_level_float,negative_tank_high_level_float,positive_tank_low_level_float,negative_tank_low_level_float,primary_stack_voltage,primary_stack_current,primary_stack_positive_pressure_sensor,primary_stack_negative_pressure_sensor,positive_stack_pressure_delta,sensor_temp,humidity,pcs1_voltage,pcs1_current,pcs1_reactive_power,pcs1_load_power,pcs1_ac_supply_power,sentToServer,markToSent,secondary_stack_positive_pressure_sensor,secondary_stack_negative_pressure_sensor,secondary_stack_voltage,secondary_stack_current,pcs1_ac_power,bcu_leakage_sensor2,dc_main_contactor,pre_charge_relay,ac_fan_coil1,fluid_fan,dcdb_fan,stk_contactor1,stk_contactor2, pcs2_voltage,pcs2_current,pcs2_reactive_power,pcs2_load_power,pcs2_ac_supply_power,pcs2_ac_power,pcs2_dc_volts,pcs2_dc_batt_current,pcs2_dc_inverter_power,pcs2_ac_out_status,pcs2_fault_status,pcs2_fan_speed,pcs2InvFreq,pcs2InternalTemperature, pcs3_voltage,pcs3_current,pcs3_reactive_power,pcs3_load_power,pcs3_ac_supply_power,pcs3_ac_power,pcs3_dc_volts,pcs3_dc_batt_current,pcs3_dc_inverter_power,pcs3_ac_out_status,pcs3_fault_status,pcs3_fan_speed,pcs3InvFreq,pcs3InternalTemperature) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);";
 ...
 //
    ...
    if (sqlite_bind_double_status(stmt, 29, current_sensor_data->battery.humidity) == -1) 
     ...
     if (sqlite_bind_int_status(stmt, 41, current_sensor_data->battery.pcs1_dc_volts) == -1) 
         return (-1); // Added on 11 Aug 21

     if (sqlite_bind_int_status(stmt, 42, current_sensor_data->battery.pcs1_dc_batt_current) == -1) 
         return (-1); // Added on 11 Aug 21

     if (sqlite_bind_int_status(stmt, 43, current_sensor_data->battery.pcs1_dc_inverter_power) == -1) 
         return (-1); // Added on 11 Aug 21
    ...
    if (sqlite_bind_double_status(stmt, 34, current_sensor_data->battery.pcs1_ac_supply_power) == -1) 
     ...
     if (sqlite_bind_int_status(stmt, 44, current_sensor_data->battery.pcs1_ac_out_status) == -1) 
         return (-1); // Added on 11 Aug 21

    if (sqlite_bind_int_status(stmt, 45, current_sensor_data->battery.pcs1_fault_status) == -1) 
         return (-1); // Added on 11 Aug 21

     if (sqlite_bind_int_status(stmt, 46, current_sensor_data->battery.pcs1_fan_speed) == -1) 
         return (-1); // Added on 11 Aug 21
    
    if (sqlite_bind_double_status(stmt, 37, current_sensor_data->battery.secondary_stack_positive_pressure_sensor) == -1) 
        return (-1);

    if (sqlite_bind_double_status(stmt, 38, current_sensor_data->battery.secondary_stack_negative_pressure_sensor) == -1) 
        return (-1);
        
    if (sqlite_bind_double_status(stmt, 39, current_sensor_data->battery.secondary_stack_voltage) == -1) 
        return (-1);

    if (sqlite_bind_double_status(stmt, 40, current_sensor_data->battery.secondary_stack_current) == -1) 
        return (-1);   
        
    if (sqlite_bind_double_status(stmt, 47, current_sensor_data->battery.pcs1_ac_power) == -1) 
        return (-1);      
        
    if (sqlite_bind_int_status(stmt, 48, current_sensor_data->battery.bcu_leakage_sensor2) == -1) 
        return (-1); 

    if (sqlite_bind_double_status(stmt, 49, current_sensor_data->battery.pcs1InvFreq) == -1) 
        return (-1); 
    if (sqlite_bind_double_status(stmt, 50, current_sensor_data->battery.pcs1InternalTemperature) == -1) 
        return (-1); 

     //PCS2 added on 9 Nov 2022
     if (sqlite_bind_int_status(stmt, 51, current_sensor_data->battery.pcs2_dc_volts) == -1) 
         return (-1);
     if (sqlite_bind_int_status(stmt, 52, current_sensor_data->battery.pcs2_dc_batt_current) == -1) 
         return (-1);
     if (sqlite_bind_int_status(stmt, 53, current_sensor_data->battery.pcs2_dc_inverter_power) == -1) 
         return (-1);
     if (sqlite_bind_double_status(stmt, 54, current_sensor_data->battery.pcs2_voltage) == -1) 
        return (-1);
     if (sqlite_bind_double_status(stmt, 55, current_sensor_data->battery.pcs2_current) == -1) 
        return (-1);
     if (sqlite_bind_double_status(stmt, 56, current_sensor_data->battery.pcs2_reactive_power) == -1) 
        return (-1);
     if (sqlite_bind_double_status(stmt, 57, current_sensor_data->battery.pcs2_load_power) == -1) 
        return (-1);
     if (sqlite_bind_double_status(stmt, 58, current_sensor_data->battery.pcs2_ac_supply_power) == -1) 
        return (-1);
     if (sqlite_bind_int_status(stmt, 59, current_sensor_data->battery.pcs2_ac_out_status) == -1) 
         return (-1);
     if (sqlite_bind_int_status(stmt, 60, current_sensor_data->battery.pcs2_fault_status) == -1) 
         return (-1);
     if (sqlite_bind_int_status(stmt, 61, current_sensor_data->battery.pcs2_fan_speed) == -1) 
         return (-1);
     if (sqlite_bind_double_status(stmt, 62, current_sensor_data->battery.pcs2_ac_power) == -1) 
        return (-1);     
     if (sqlite_bind_double_status(stmt, 63, current_sensor_data->battery.pcs2InvFreq) == -1) 
        return (-1); 
     if (sqlite_bind_double_status(stmt, 64, current_sensor_data->battery.pcs2InternalTemperature) == -1) 
        return (-1); 


     //PCS3 added on 9 Nov 2022
     if (sqlite_bind_int_status(stmt, 65, current_sensor_data->battery.pcs3_dc_volts) == -1) 
         return (-1);
     if (sqlite_bind_int_status(stmt, 66, current_sensor_data->battery.pcs3_dc_batt_current) == -1) 
         return (-1);
     if (sqlite_bind_int_status(stmt, 67, current_sensor_data->battery.pcs3_dc_inverter_power) == -1) 
         return (-1);
     if (sqlite_bind_double_status(stmt, 68, current_sensor_data->battery.pcs3_voltage) == -1) 
        return (-1);
     if (sqlite_bind_double_status(stmt, 69, current_sensor_data->battery.pcs3_current) == -1) 
        return (-1);
     if (sqlite_bind_double_status(stmt, 70, current_sensor_data->battery.pcs3_reactive_power) == -1) 
        return (-1);
     if (sqlite_bind_double_status(stmt, 71, current_sensor_data->battery.pcs3_load_power) == -1) 
        return (-1);
     if (sqlite_bind_double_status(stmt, 72, current_sensor_data->battery.pcs3_ac_supply_power) == -1) 
        return (-1);
     if (sqlite_bind_int_status(stmt, 73, current_sensor_data->battery.pcs3_ac_out_status) == -1) 
         return (-1);
     if (sqlite_bind_int_status(stmt, 74, current_sensor_data->battery.pcs3_fault_status) == -1) 
         return (-1);
     if (sqlite_bind_int_status(stmt, 75, current_sensor_data->battery.pcs3_fan_speed) == -1) 
         return (-1);
     if (sqlite_bind_double_status(stmt, 76, current_sensor_data->battery.pcs3_ac_power) == -1) 
        return (-1);     
     if (sqlite_bind_double_status(stmt, 77, current_sensor_data->battery.pcs3InvFreq) == -1) 
        return (-1); 
     if (sqlite_bind_double_status(stmt, 78, current_sensor_data->battery.pcs3InternalTemperature) == -1) 
        return (-1); 
...
int sensorDataCallback(void *sensorDataPtr, int argc, char **argv, char **azColName)
{
    ...
    for(i = 0; i < argc; i++) {
       ...
       else if(strcmp(azColName[i],"bcu_leakage_sensor2") == 0)  // Added on 11 Aug 21
        ...
        else if(strcmp(azColName[i],"dc_main_contactor") == 0)                     // Added 06-12-2021
            fetched_sensor_data->battery.dc_main_contactor = atof(argv[i]);        // Added 06-12-2021
        else if(strcmp(azColName[i],"pre_charge_relay") == 0)                             // Added 06-12-2021
            fetched_sensor_data->battery.pre_charge_relay = atof(argv[i]);                // Added 06-12-2021
        else if(strcmp(azColName[i],"ac_fan_coil1") == 0)                 // Added 06-12-2021
            fetched_sensor_data->battery.ac_fan_coil1 = atof(argv[i]);    // Added 06-12-2021
